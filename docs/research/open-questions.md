...
|---|---|
| 状态 | **`OPEN`（法律问题-待确认）** |
| 阻塞 | 内核二进制的获取渠道与 deb 打包流程（Bundled 的核心前提）。 |
| 背景 | 本机实测 `github.com` 直连超时，必须走 `ghfast.top` 等代理；但经第三方镜像下载再分发给用户，涉及 GPL-3.0 §6 的"对应源码提供"与渠道可信性。 |
| 建议 | 优先从上游 release 直接获取并校验 checksum；若必须用镜像，则在文档中明确记录来源与校验值；Bundled 时履行 §6(d)（同址提供对应 tag 源码归档）。 |
| 关联 | R13 §8 待确认清单、ADR-006 D4 |
...
| 状态 | 数量 | 条目 |
|---|---|---|
| `RESOLVED` | 10 | Q002、Q003、Q005（方向）、Q006、Q007、Q008、Q010、Q013、Q014、Q001(PARTIAL→已定实现方式) |
| `PARTIAL` | 9 | Q001、Q004、Q011、Q017、Q018、Q019、Q021、Q022、Q025 |
| `OPEN` | 6 | Q009、Q012、Q015、Q016、Q020、Q023、Q024 |
...
```text
Q015/Q016  → 打包与内嵌前（ADR-006 D4/D1）
