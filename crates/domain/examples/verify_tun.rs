...
use proxy_domain::system::capability::{
    evaluate_tun, CapabilityKind, CapabilityStatus, ProbeResult,
};
...
fn main() {
    ...
    // TUNSETIFF EPERM).
    let without_cap = evaluate_tun(true, true, CapabilityStatus::Unavailable, ProbeResult::PermissionDenied, now);
    println!("without CAP_NET_ADMIN -> {:?} (expect Misconfigured)", without_cap.status());
    ...
    let with_cap = evaluate_tun(true, true, CapabilityStatus::Supported, ProbeResult::Ok, now);
    println!("with CAP_NET_ADMIN    -> {:?} (expect Supported)", with_cap.status());
    ...
    let absent = evaluate_tun(false, false, CapabilityStatus::Unavailable, ProbeResult::Missing, now);
    println!("device absent         -> {:?} (expect Unavailable)", absent.status());
