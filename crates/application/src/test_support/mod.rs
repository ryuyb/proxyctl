...
// Panicking is acceptable in this module only: it is test scaffolding, and a
// double that cannot build its own fixture should fail loudly rather than
// thread `Result` through every test call site. The ban in `lib.rs` applies to
// the use cases, not to this module.

// Panicking helpers are acceptable here and only here: this module is test
// scaffolding, and a double that cannot build its own fixture should fail loudly
// rather than thread `Result` through every test call site. The ban on panicking
// in `lib.rs` applies to the use cases, not to this module.
//! In-memory test doubles with fault injection.
//!
//! These exist so the activation and update paths can be exercised against every
//! failure mode without a kernel, a database, or a network. Each double can be
//! told to fail at a specific call, which is what makes recovery paths reachable
//! from tests rather than only from production incidents.
...
impl Harness {
    ...
    /// Sets the active version directly, simulating prior state.
    ///
    /// Also stores a matching version record, so `active()` can resolve it.
    pub fn set_active(&self, id: &str) {
        ...
        if let Ok(mut versions) = self.configs.versions.lock() {
            versions.push(version);
        }
        if let Ok(mut active) = self.configs.active.lock() {
            *active = Some(version_id);
        }
    }
